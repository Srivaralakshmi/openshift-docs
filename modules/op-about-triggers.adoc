...
* The `Trigger` resource combines the `TriggerBinding` and `TriggerTemplate` resources, and the `interceptors` event processor. 
+
Interceptors process all the events for a specific platform that runs before the TriggerBinding resource. They allow you to: perform payload filtering, verification, transformation, define and test trigger conditions, and implement other useful processing. Interceptors use Secret for verification. Once the event data passes through an interceptor, it then goes to the trigger before you pass the payload data to the trigger binding. You can also use an interceptor to modify the behavior of the associated trigger. This `Trigger` resource is referenced in the `EventListener` specification.
